    ...
    class PrintSim
    {
        ...
        static string HostToFirmware(byte[] packet, PrinterControl simCtl)
        {
            ...
            if (header == possible_header)
            {
                ...
                byte[] response = { 0x00 };
                int response_bytes = simCtl.ReadSerialFromFirmware(response, response_size) 
                while ()
                {
                    // wait
                }
                ...
                // string response_to_string = something;
                if (response == "SUCCESS" || response == "VERSION n.n")
                {
        ...
        static string FirmwareToHost(byte[] packet)
        {
            ...
            if (ACK_NAK == ACK)
            {
                ...
                else
                {
                    ...
                    byte[] check_checksum = { 0x00 };   // compare bytes 2 and 3 from both the header and what is recieved by adding up both parts: header and rest
                    //      If checksum correct
                    if ()
                    {
